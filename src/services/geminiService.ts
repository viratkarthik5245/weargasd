...
// Fallback for when ENV variables aren't loaded in deployment
const FALLBACK_OPENROUTER_API_KEY = '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';
const OPENROUTER_API_KEY = import.meta.env.VITE_OPENROUTER_API_KEY || FALLBACK_OPENROUTER_API_KEY;

console.log('GeminiService: OpenRouter API Key:', import.meta.env.VITE_OPENROUTER_API_KEY ? 'FROM ENV' : 'USING FALLBACK');
...
${additionalSections && additionalSections.length > 0 ? `Additional Sections Provided: ${JSON.stringify(additionalSections)}` : ''}`;
  ...
  const response = await safeFetch({
    ...
    body: JSON.stringify({
      model: "google/gemini-2.5-flash-lite", // Set to google/gemini-flash-1.5
...
Return ONLY a JSON array of strings: ["skill1", "skill2", "skill3", "skill4", "skill5"]`;
  ...
  const response = await safeFetch({
    ...
    body: JSON.stringify({
      model: modelOverride || 'google/gemini-2.5-flash-lite', // Set to google/gemini-2.5-flash-lite
